...
class PrintAgentApp {
    constructor() {
        ...
        this.horizontalOffset = 0; // + вправо, - влево (в мм)
        this.verticalOffset = 0; // + вверх, - вниз (в мм)
        this.init();
    }
    async init() {
    ...
    setupEventListeners() {
        ...
        const restaurantCodeInput = document.getElementById('restaurant-code');
    ...
    setupMainProcessListeners() {
        ...
        window.electronAPI.onPrintJobReceived((job) => {
            ...
            this.showNotification(`🖨️ Получена этикетка "${job.labelData.category}" от ${job.labelData.preparerName}`, 'info');
    ...
    async testPrinter(printerName) {
        const testBtn = document.querySelector(`[data-printer="${printerName}"]`);
        if (testBtn) {
    ...
    async handleConnect() {
        ...
        const agentTokenInput = document.getElementById('agent-token');
        const connectBtn = document.getElementById('connect-btn');
        if (!restaurantCodeInput || !connectBtn || !agentTokenInput)
            return;
    ...
    updateLabelStatus(jobId, status, errorMessage) {
    ...
    updateOffsetInputs() {
        ...
        const horizontalInput = document.getElementById('horizontal-offset-input');
        if (horizontalInput) {
            horizontalInput.value = this.horizontalOffset.toFixed(1);
        }
        const verticalInput = document.getElementById('vertical-offset-input');
        if (verticalInput) {
